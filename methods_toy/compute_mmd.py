...
def dfs_data(args):
    ...
    for i in pbar:
        ...
        hamming_mmd, bandwidth = exp_hamming_mmd(x,y,args,bandwidth=args.bandwidth)
        euclidean_mmd, sigma = rbf_mmd(x,y,args,sigma=args.sigma)
        exp_hamming_mmd_list.append(hamming_mmd)
        rbf_mmd_list.append(euclidean_mmd)
    hamming_mmd = sum(exp_hamming_mmd_list)/args.num_rounds
    euclidean_mmd = sum(rbf_mmd_list)/args.num_rounds
    log_mmd(hamming_mmd, bandwidth, euclidean_mmd, sigma, args)
...
def log_mmd(hamming_mmd, bandwidth, euclidean_mmd, sigma, args):
    ...
    log_entry['hamming_mmd'] = hamming_mmd.item()
    log_entry['bandwidth'] = bandwidth.item()
    log_entry['euclidean_mmd'] = euclidean_mmd.item()
    log_entry['sigma'] = sigma.item()
