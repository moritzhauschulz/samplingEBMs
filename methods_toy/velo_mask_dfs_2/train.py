...
from velo_mask_dfs_2.model import MLPScore, EBM
from velo_mask_dfs_2.model import MLPModel as MLPFlow
from velo_mask_dfs_2.model import alt_MLPModel as alt_MLPFlow
...
def compute_loss(ebm_model, dfs_model, q_dist, xt, x1, t, args):
    ...
    u_t_target_1 = torch.zeros((B, D, S)).to(args.device)
    u_t_target_1 = u_t_target_1.scatter_(-1, x1[:, :, None], 1.0)

    u_t_target_2 = torch.zeros((B, D, S)).to(args.device)
    u_t_target_2 = u_t_target_2.scatter_(-1, xt[:, :, None], 1.0)

    u_t_target = 1/(1-t_expanded) * (u_t_target_1 - u_t_target_2)
    
    #should I multiply by 1-t?

    loss = (u_t_target - u_t).square()
    #loss.scatter_(-1, xt[:, :, None], 0.0) #because the self-connections are imputed (?)
...
def main_loop(db, args, verbose=False):
    ...
    for epoch in pbar:
        ...
        if (epoch % args.eval_every == 0) or (epoch == args.num_epochs):
            ...
            ebm_model.eval()
            log_entry['sampler_hamming_mmd'], log_entry['bandwidth'], log_entry['sampler_euclidean_mmd'], log_entry['sigma'] = sampler_evaluation(args, db, lambda x: torch.from_numpy(gen_samples(dfs_model, args, batch_size=x)))
            log_entry['sampler_ebm_hamming_mmd'], log_entry['bandwidth'],log_entry['sampler_ebm_euclidean_mmd'], log_entry['sigma'] = sampler_ebm_evaluation(args, db, lambda x: torch.from_numpy(gen_samples(dfs_model, args, batch_size=x)), ebm_model)
