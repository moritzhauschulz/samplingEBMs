...
from velo_mask_dfs_ce_4.model import MLPScore, EBM
from velo_mask_dfs_ce_4.model import MLPModel as MLPFlow
...
def compute_loss(ebm_model, dfs_model, q_dist, xt, x1, t, args):
    ...
    log_q_density = q_dist.log_prob(x1.float())
    log_q_density = log_q_density.sum(dim=-1)
    log_weights = log_prob - log_q_density
    log_norm = torch.max(log_weights) #make this a moving average?
    weights = (log_weights - log_norm).exp()
    output_dir = f'{args.plot_path}/weights_histogram.png'
