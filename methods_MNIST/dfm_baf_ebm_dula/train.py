...
def get_x_fake(B,D,S,epoch,itr, x, dfs_model, ebm_model, args):
    if args.rand_t:
        ...
        t_target = 1 - min(1, (args.itr_per_epoch * (epoch - 1) + itr)/ (args.itr_per_epoch * args.warmup_baf))
    elif args.t > 0:
        t_target = args.t
    else:
        raise ValueError
...
def main_loop_real(args, verbose=False):
    ...
    if args.buffer_init == "mean":
        if args.input_type == "binary":
            buffer = init_dist.sample((args.buffer_size,)).cpu()
        else:
            buffer = None
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        for itr, ((x, _), (x_source, _)) in enumerate(zip(pbar, cycle(source_train_loader))):
            ...
            x = preprocess(x).long().to(args.device)
            ...
            dfs_loss, acc, success_rate, t_target = torch.tensor(0.0), 0.0, 0.0, 0.0
        ...
        if verbose:
            print(f'Epoch: {epoch}\{args.num_epochs}; Final DFS Loss: {dfs_loss}; EBM Loss: {ebm_loss}; mean logp_real: {logp_real.mean().item()}; mean logp_fake: {logp_fake.mean().item()}; \n')
