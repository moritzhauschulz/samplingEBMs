...
from velo_uni_efm.model import ResNetFlow
from velo_uni_efm.model import EBM
from velo_uni_efm.model import Dataq
...
#     """
#     Apply the log-sum-exp trick to stabilize computations involving log-sums.
#     """
#     max_tensor = torch.max(tensor, dim=dim, keepdim=True)[0]
#     return max_tensor + torch.log(torch.sum(torch.exp(tensor - max_tensor), dim=dim, keepdim=True))
...
def main_loop(args, verbose=False):
    ...
    for epoch in range(args.num_epochs):
        ...
        for it, (x, _) in enumerate(pbar):
            ...
            loss, acc, weights = compute_loss(ebm_model, temp, dfs_model, q_dist, xt, x1, t, args)
